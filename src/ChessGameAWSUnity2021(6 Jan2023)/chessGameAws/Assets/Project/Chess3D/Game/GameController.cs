...
namespace Assets.Project.Chess3D
{
    public class GameController : MonoBehaviour
    {
	    ...
	    public AudioClip audioPiecePick, audioPieceBack,audioPieceDrop ;
        ...
        private void Update()
        {
            ...
            {
                dt = 0;
                int childcount = Spawner.Pieces.transform.childCount;
                int rndChild = Random.Range(0, childcount);

                Animator amt = Spawner.Pieces.transform.GetChild(rndChild).GetComponent<Animator>();
                if (amt.GetBool("attack1") || amt.GetBool("attack2")) return;
                amt.SetTrigger("idl" + Random.Range(1, 5));
            }
        }
        ...
        IEnumerator PlayGame()
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    {
                        UiController.ShowInputInfoText(OnTurn.Id + ", select piece to move.");
                        if (dt > 5000)
                        {
                            dt = 0;
                    ...
                    {
                        UiController.ShowInputInfoText("<color=yellow>" + OnTurn.Id + ", select square to move." + "</color>");
                        if (dt > 5000)
                        {
                            dt = 0;
                            Spawner.SelectedPieceAni(SelectedPiece);
                        }
                    }
        ...
        public void SelectedPieceAni()
        {
	        Spawner.SelectedPieceAni(SelectedPiece);
	        gameObject.GetComponent<AudioSource>().clip = audioPiecePick;
	        gameObject.GetComponent<AudioSource>().Play();
        }
        public void DeselectedPieceAni()
        {
	        Spawner.DeselectedPieceAni(SelectedPiece);
	        gameObject.GetComponent<AudioSource>().clip = audioPieceBack;
	        gameObject.GetComponent<AudioSource>().Play();
        }
        ...
        public void DoMove(int sq120)
        {
            ...
            if (Board.MoveExists(foundMove))
            {
	            ...
	            gameObject.GetComponent<AudioSource>().clip = audioPieceDrop;
	            gameObject.GetComponent<AudioSource>().Play();
