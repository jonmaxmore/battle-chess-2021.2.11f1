...
namespace Assets.Project.Chess3D
{
    public class Spawner: MonoBehaviour
    {
        ...
        public void RandomIdleAni()
        {
            

        }
        public void SelectedPieceAni(Piece piece)
        {
            PieceWrapper wrapper = FindPieceWrapper(piece);
            wrapper.gameObject.GetComponent<Animator>().SetBool("attack" + UnityEngine.Random.Range(1, 2), true);

            Material[] mats = wrapper.header.GetComponent<SkinnedMeshRenderer>().materials;
            mats[0] = selectedMt;
            wrapper.header.GetComponent<SkinnedMeshRenderer>().materials = mats;

        }
        public void DeselectedPieceAni(Piece piece)
        {
            PieceWrapper wrapper = FindPieceWrapper(piece);
            wrapper.gameObject.GetComponent<Animator>().SetBool("attack1", false);
            wrapper.gameObject.GetComponent<Animator>().SetBool("attack2", false);
        ...
        private Vector3 ToWorldPoint(int cellNumber)
        {
            int j = cellNumber % 8;
            int i = cellNumber / 8;
	        return new Vector3(i * -7.0f + 14 * 1.85f, 1, j * 7.5f  - 14 * 1.85f);
        }
    }
}
